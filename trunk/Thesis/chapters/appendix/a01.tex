...
As opções para o parâmetro \verb|limiter| são: \verb|SUPG| (ou seja, apenas o termo de estabilização), \verb|SUPG+Wells| (estabilização via \ac{SUPG} e adição de difusão artificial) e \verb|none| (\ac{MEF} de Galerkin aplicado diretamente para a equação hiperbólica). Em relação ao tipo de limitador para captura de choque, até o momento apenas o termo de difusão artificial isotrópica sugerido em \citep{Wells2008} está disponível, porém um dos trabalhos futuros (ver Seção \ref{sc:trabFuturos}) é exatamente a adição de difusão artificial anisotrópica conforme proposto por \cite{Codina1992}.
...
Sendo que o arquivo do tipo \verb|*.geo| para gerar a geometria desejada é descrito na listagem a seguir: 
