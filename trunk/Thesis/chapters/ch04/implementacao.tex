...
A formulação variacional para a equação da saturação utilizando o \ac{MEF} com estabilização via \ac{SUPG} é representada matematicamente da seguinte forma:
...
def SUPG(self, delfineVar, parameter):
    ...
    F = v*phi*((S-S0)/dt)*dx +
        (v*fs*dot(velocity, grad(S))*dx
    # Residual
    r = phi*((S-S0)/dt) + fs*(dot(velocity, grad(S)))
