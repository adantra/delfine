...
\institute{Seminário de Dissertação - Mestrado}
%%\titlegraphic{\includegraphics[width=20mm]{Reservoir-SimulationFigure}}
\date{Fevereiro - 2012}
\AtBeginSection[]
{
   \begin{frame}
...
\begin{frame}
  ...
  \begin{block}{Simulação de Reservatórios de Petróleo}
   \begin{itemize}
    \item <2-> Agilidade no Desenvolvimento e Gerenciamento de Reservatórios
    \item <3-| alert@3-> Métodos Numéricos Robustos e Eficientes
   \end{itemize}
  \end{block}

  \begin{block}{Programas de Simulação Numérica}
   \begin{itemize}
    \item <4-> Codificação dos Métodos pode ser Complexa e Demorada
    \item <5- | alert@5-> Bibliotecas para Automação da Geração do Código
   \end{itemize}
  \end{block}
  
  \begin{block}{Eficiência na Resolução de Sist. Eq. Lineares}
   \begin{itemize}
    \item <6-> Velocidade de simulação é crucial para viabilizar uso
    \item <7- | alert@7-> Métodos de Aceleração de Convergência
   \end{itemize}
  \end{block}
\end{frame}
...
\subsection{Elíptico - Homogêneo e Anisotrópico}
...
\begin{frame}
  \frametitle{Evolução do Resíduo e do Erro}
  \begin{columns}[c]
...
\begin{frame}
  \frametitle{Solução Numérica - Preliminar}
\begin{figure}
\centering
\subfigure{
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{figs/SaturationHeteroIso2}
\caption{Oscilações no campo de Saturação para t = 0.5 PVI}
\end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Desenvolvimento rápido de simulador de escoamentos em meios porosos.
    \item Facilidade de acrescentar novos métodos ou acessórios no código.
    \item Utilizadas diferentes metodologias do MEF (Galerkin, MEFM, MEF+SUPG).
    \item SUPG fundamental para estabilizar a parte hiperbólica.
