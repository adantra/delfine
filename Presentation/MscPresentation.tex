...
\frametitle{Equação da Saturação}
\begin{itemize}
\item <1-> Assumindo novamente as hipóteses de incompressibilidade do meio e do fluido, desprezando a pressão capilar e considerando um fluxo horizontal:
\end{itemize}
...
\end{itemize}
  \begin{figure} 
  \centering
  \includegraphics[width=0.75\textwidth]{figs/fracflux}
  \end{figure}
\end{frame}
...
\end{equation*}
  ...
  \begin{block}{Produto interno em $L_{2}(\Omega)$}<1->
...
\end{equation*}
  ...
  \begin{block}{Multiplicando a Eq. da velocidade por $\mathbf{\zeta} \in \mathbf{V}$ e rearranjando os termos}<1->
\begin{equation*} \label{eq:pressure_eq_formMFEM4}
\left( \frac{\mathbi{K}^{-1}}{\lambda} \mathbf{v}, \zeta \right) = (\nabla \cdot \zeta, p)
\end{equation*}
  \end{block}
  
  \begin{block}{Multiplicando a Eq. da pressão por um $w \in W = L_{2}(\Omega)$}<1->
...
\item <1-> Presença de instabilidades para equações hiperbólicas. 
\item <1-> Ponderação com peso maior na direção do fluxo (Idéia do \emph{Upwind})
...
\begin{frame}
  ...
  \begin{figure}
  \centering
  \includegraphics[width=0.75\textwidth]{figs/BL1D}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}
  \centering
  \includegraphics[width=0.85\textwidth]{figs/BL1Dcomp}
  \end{figure}
\end{frame}
