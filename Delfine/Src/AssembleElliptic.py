...
class AssembleElliptic:
    ...
    def __init__(self, parameter,  meshData):
        ...
        self.mob = mobW + mobO
############################################################
    def Galerkin(self, delfineVar, parameter,  meshData):
...
#        bc = DirichletBC(V, Expression("1.0"), u1_boundary,  "pointwise") 
                ...
                for well in wells:
                    if (mf[vertex.index()] == int(well.id)):
                        if (int(well.id) > 300): # Source/Sink Well
                            ...
                            q.apply(rhs)
        # FIXME: Pressure/Saturation Well still not implemented
        ...
        delfineVar.u0 = u0
############################################################
    def MixedFEM(self, delfineVar):
        """Assembles the elliptic problem in a mixedFEM variational form with Dolfin.""" 
        ...
        a = (dot(sigma, tau) - div(tau)*u - div(sigma)*v)*dx # FIXME: teria que ser inv(K)*inv(mob)
        L = - f*v*dx
        ...
        p = Point(0.0, 0.0)
        q = PointSource(W.sub(1), p, -1.0)  #FIXME: Verificar Sources!!
        q.apply(rhs)
        
        p2 = Point(1.0, 1.0)
        q2 = PointSource(W.sub(1), p2, 1.0) 
        q2.apply(rhs)
