...
\end{array} \right) & \textrm{se $x > 0$}
  \end{array} \right.
\end{equation}
onde o fator $\alpha$ é um parâmetro utilizado para definir a intensidade de descontinuidade em $x = 0$.
...
A malha não-estruturada utilizada para $N \simeq 8$ e a divisão dos subdomínios em $x=0$ pode ser observada na figura \ref{fig:HeterogeneoAnisoMaterials_N=8}.

\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{chapters/ch05/HeterogeneoAnisoMaterials_N=8}
\caption[Caso heterogêneo e anisotrópico. Malha não-estruturada e heterogeneidade]{Domínio utilizado e malha não-estruturada com $N \simeq 8$ e heterogeneidade em $x=0$.}
\label{fig:HeterogeneoAnisoMaterials_N=8}
\end{figure}

Pode ser observado na figura \ref{fig:HeterogeneoAnisoContour_N=64} o efeito da descontinuidade na linha $x=0$, efeito este que se torna mais pronunciado ao aumentarmos o valor do parâmetro $\alpha$. Os valores para os erros da solução numérica quando comparada com a analítica para as normas do máximo e $L2$ são apresentados na tabela \ref{tab:heteroiso}, onde pode ser visto que um aumento na descontinuidade do tensor $\mathbf{K}$ para os diferentes sub-domínios causa um aumento nos valores absolutos dos erros obtidos.
...
\caption[Caso heterogêneo e anisotrópico. Campo escalar para malha de 64x64.]{Campo escalar obtido para diferentes fatores de descontinuidade $\alpha$ utilizando o MEF com malha de 64x64 discretizada por triângulos lineares.}
...
Um fato interessante que pode ser constatado a partir dos valores obtidos para os erros da solução numérica obtida é que os mesmos apresentam uma taxa de convergência em torno de 1 para $\alpha = 1$ e $10$, sendo que esta taxa se aproxima de 2 quando aumentamos o fator de descontinuidade ($\alpha = 100$ e $1000$).
...
Os resultados obtidos para a evolução dos resíduos podem ser verificados na figura \ref{fig:residualHeterogeneoAniso_N=64_alpha=1}, onde mais uma vez é constatada a superioridade dos métodos que utilizam multigrid, mesmo para este problema que apresenta sub-domínios com propriedades diferentes.

\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{chapters/ch05/residualHeterogeneoAniso_N=64_alpha=1}
\caption[Caso heterogêneo e anisotrópico. Comparação da evolução dos resíduos para malha 64x64.]{Caso heterogêneo e anisotrópico. Comparação da evolução dos resíduos para malha 64x64 discretizada por triângulos lineares e com $\alpha = 1$.}
