...
Assim, podemos definir o espaço $\mathbf{V} = \{\mathbf{\zeta} \in \mathbf{H}(div, \Omega); \mathbf{\zeta} \cdot n = 0 \phantom{l} \textrm{em $\Gamma_{N}$} \}$ para garantir a condição de fluxo zero na fronteira e o $W = L_{2}(\Omega)$, onde é importante observar que funções em $W$ não são necessariamente contínuas.
