...
Várias combinações de espaços que satisfazem a condição de estabilidade conhecida como \emph{Ladyshenskaja-Babuska-Brezzi Condition} (LBB) (ver \cite{Brezzi1991} para uma discussão detalhada sobre esta condição) já foram sugeridos na literatura \citep{Brezzi1985, Chen2006} para problemas elípticos de 2ª ordem como o representado pela equação da pressão (Eq. (\ref{eq:pressure_eq_form3})).

Para isto, é necessário reescrever a  descrição matemática da equação de pressão, subdividindo a mesma em dois problemas, como pode ser visto na Eq. (CITAR EQ. ABAIXO).
