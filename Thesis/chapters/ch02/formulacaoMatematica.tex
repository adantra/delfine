...
Uma vez obtido o valor para a saturação normalizada, as permeabilidade relativas para as fases são calculadas de acordo com a expressão a seguir. Um exemplo do uso do modelo de Corey para o cálculo das permeabilidades relativas é apresentado na Fig. \ref{fig:relat_perm}.
...
\phi\frac{\partial C}{\partial t} = - \nabla\cdot\left(C\mathbi{v} - \mathbi{D}\nabla C \right) + \hat{C}Q
\end{equation}%
...
No Capítulo \ref{ch:form_num} a versão unidimensional em meio homogêneo da Eq. (\ref{eq:misc1}) será utilizada para a apresentação de alguns dos métodos numéricos utilizados neste trabalho. Resultados para casos multidimensionais e considerando meios altamente heterogêneos serão apresentados e discutidos no Capítulo \ref{ch:resultados} esta equação modelo.
